...
var boilerplate = function (gulp, opts) {
  ...
  if (opts.test) {
    var testFiles = _.flatten([opts.test.files || opts.testFiles]).map(function (f) {
      return interpolate(f, {testDir: testDir});
    });
  ...
  if (opts.e2eTest) {
    ...
    gulp.task('e2e-test', testDeps,  function () {
      ...
      if (opts.e2eTest.androidEmu) {
        var emu = new AndroidEmulator(opts.e2eTest.androidAvd);
        seq.unshift(function () {
          emu.start();
          return emu.waitTillReady();
        });
        seq.push(function () { return emu.stop(); });
      }
      if (opts.e2eTest.xCode) {
        var xCodeVersion = opts.e2eTest.xCodeVersion || '6.1.1';
        seq.unshift(function () {
          return iosTools.configureXcode(xCodeVersion).then(function () {

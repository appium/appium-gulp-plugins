    ...
    gulp.task('coverage', function () {
      return exec('rm -rf ./build').then(function () {
        return globby(covTestFiles).then(function (files) {
          ...
          var args = [bins.isparta, 'cover', bins._mocha, '--', '--require', __dirname + '/../babelhook', '--compilers', 'babel:babel/register' , '--reporter', 'dot']
            .concat(files);
      ...
      if (opts.e2eTest.android) {
        if(opts.e2eTest["android-emu"]) {
          ...
          cleanupSeq = cleanupSeq.concat([
            function () { return emu.stop().catch(cleanupWarn); },

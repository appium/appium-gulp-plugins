...
let boilerplate = function (gulp, opts) {
      ...
      return exec('rm -rf ./build').then(function () {
        return exec('npm bin').then(function (npmBin) {
          ...
          return globby(covTestFiles).then(function (files) {
            ...
            let bins = {};
            _.each(['isparta', 'babel-node', 'text', '_mocha'], function (k) { bins[k] = path.resolve(npmBin, k); });
            let bin = bins['babel-node'];
            let args = [bins.isparta, 'cover', '--dir', targetDir, bins._mocha, '--', '--require', __dirname + '/../babelhook', '--reporter', 'dot']
              .concat(files);
  ...
  if (opts.coverage) {
    ...
    gulp.task('coveralls', ['coverage'], function () {
      let bin = path.resolve(__dirname, '../node_modules/.bin/coveralls');

    ...
    gulp.task('coverage', function () {
      return exec('rm -rf ./build').then(function() {
        return globby(covTestFiles).then(function (files) {
          ...
          var bins = {};
          _.each(['babel-node', 'isparta', '_mocha'], function (k) { bins[k] = path.resolve(__dirname, '../node_modules/.bin', k); });
          var bin = bins['babel-node'];
          var args = [ '--ignore', 'node_modules', '-r3', bins.isparta, 'cover',
            '--report', 'text', bins._mocha, '--', '--reporter', 'dot']
            .concat(files);
          var proc = spawn(bin, args, {stdio: opts.coverage.verbose ? 'inherit' : 'pipe'});
